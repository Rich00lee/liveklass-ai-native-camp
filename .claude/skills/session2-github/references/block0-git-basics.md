...
한 마디로 **Google Docs의 코드 버전**이라고 생각하면 됩니다. Google Docs가 문서를 클라우드에 저장하고 어디서든 편집할 수 있게 해주듯, GitHub은 코드와 설정 파일을 클라우드에 저장하고 어디서든 접근할 수 있게 해줍니다.

비유하면:
| 개념 | 비유 |
|------|------|
| Git | 내 컴퓨터의 세이브 파일 |
| GitHub | 클라우드 세이브 (어디서든 이어하기). Google Docs처럼 온라인 저장 |
| commit | 세이브하기 |
| push | 클라우드에 업로드 |
| pull | 클라우드에서 다운로드 |
...
시작하기 전에 Claude에게 환경부터 확인시키세요:

```
git이 설치되어 있는지, gh CLI가 인증되어 있는지 확인해줘.
```

> Claude가 자동으로 `git --version`, `gh auth status` 등을 실행해서 결과를 알려줍니다.
> 문제가 있으면 Claude가 해결 방법도 안내해줍니다. 모두 통과하면 아래로 진행!
...
### 막히면? git-onboarding 플러그인

Git 설정이 막히거나 어려우면, Claude에게 이렇게 말해보세요:

```
/git-onboarding-auto
```

이 플러그인은 환경 점검 → Git 설정 → 파일 생성 → 브랜치 → PR 생성까지 **한 번에 자동 처리**합니다. 이미 설정이 된 부분은 건너뛰고 필요한 단계만 진행해요.

그 외 도움이 필요하면:
- `/git-onboarding-help` — Git 용어 설명과 FAQ
- `/git-onboarding-step` — 현재 진행 상황 체크리스트

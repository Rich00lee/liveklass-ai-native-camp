...
📋 ATS에서 후보자 정보 + 이력서 수집
→ 📅 캘린더 빈 시간 확인 → 제안 메시지 자동 생성·발송
→ 💬 답변 감지 → 수락 시 확정 메시지 + 캘린더 등록
→ 📝 이력서 + JD 기반 맞춤 질문 → **노션 기록지 생성**
...
| 서비스 | 역할 |
|------|------|
| 그리팅 ATS | 후보자 정보 + 이력서 조회 |
| Google Calendar | 빈 시간 탐색 + 일정 등록 |
| Apple Messages | SMS 우선 발송 + 답변 감지 |
| Notion | 맞춤 질문 + 인터뷰 기록지 생성 |

**실사용 중 발견하고 개선한 것**
- 매 단계 승인 버튼 10회 → 실제로 돌려보니 클릭만 하다 끝남 → **에이전트 자율 실행**으로 전환
- iMessage 전송 실패 → 후보자가 안드로이드면 발송 자체가 안 됨 → **SMS 우선 발송**으로 변경
...
| 과제 | 설명 |
|------|------|
| 다이렉트 소싱 | 링크드인·원티드 등 플랫폼별 후보자 제안 발송 자동화 |
| 대면 인터뷰 어레인지 | 현업 면접관 + 후보자 일정 조율 → 캘린더·회의실·안내 메시지 |
| 다중 후보자 일괄 처리 | 동시에 여러 명에게 제안 메시지 발송 |

> "후보자와의 직접 통화를 제외한 모든 오퍼레이션을 자동화하겠다."
